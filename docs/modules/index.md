[inherits-ex](../README.md) / [Exports](../modules.md) / index

# Module: index

## Table of contents

### References

- [CloneFilterFn](index.md#clonefilterfn)
- [FilterFn](index.md#filterfn)
- [InheritsEx](index.md#inheritsex)
- [\_clone](index.md#_clone)
- [\_extend](index.md#_extend)
- [createCtor](index.md#creatector)
- [createFunction](index.md#createfunction)
- [createObject](index.md#createobject)
- [createObjectWith](index.md#createobjectwith)
- [defineProperty](index.md#defineproperty)
- [extend](index.md#extend)
- [filterOpts](index.md#filteropts)
- [getClassByName](index.md#getclassbyname)
- [getConstructor](index.md#getconstructor)
- [getCtorOfOwnProperty](index.md#getctorofownproperty)
- [getOwnPropValue](index.md#getownpropvalue)
- [getParentClass](index.md#getparentclass)
- [getProtoChain](index.md#getprotochain)
- [getPrototypeOf](index.md#getprototypeof)
- [getSuper](index.md#getsuper)
- [getSuperCtor](index.md#getsuperctor)
- [hasNativeReflect](index.md#hasnativereflect)
- [inherits](index.md#inherits)
- [inheritsDirectly](index.md#inheritsdirectly)
- [inheritsObject](index.md#inheritsobject)
- [isEmptyCtor](index.md#isemptyctor)
- [isEmptyFunction](index.md#isemptyfunction)
- [isEmptyFunctionCli](index.md#isemptyfunctioncli)
- [isInheritedFrom](index.md#isinheritedfrom)
- [isInheritedFromStr](index.md#isinheritedfromstr)
- [isMixinedFrom](index.md#ismixinedfrom)
- [isMixinedFromStr](index.md#ismixinedfromstr)
- [isNativeReflectConstruct](index.md#isnativereflectconstruct)
- [mixin](index.md#mixin)
- [mixins](index.md#mixins)
- [replaceCtor](index.md#replacector)
- [setPrototypeOf](index.md#setprototypeof)

## References

### CloneFilterFn

Re-exports [CloneFilterFn](clone.md#clonefilterfn)

___

### FilterFn

Re-exports [FilterFn](mixin.md#filterfn)

___

### InheritsEx

Re-exports [InheritsEx](inherits_ex.md#inheritsex)

___

### \_clone

Re-exports [_clone](clone.md#_clone)

___

### \_extend

Re-exports [_extend](extend.md#_extend)

___

### createCtor

Re-exports [createCtor](createCtor.md#creatector)

___

### createFunction

Re-exports [createFunction](createFunction.md#createfunction)

___

### createObject

Re-exports [createObject](createObject.md#createobject)

___

### createObjectWith

Re-exports [createObjectWith](createObjectWith.md#createobjectwith)

___

### defineProperty

Re-exports [defineProperty](defineProperty.md#defineproperty)

___

### extend

Re-exports [extend](extend.md#extend)

___

### filterOpts

Re-exports [filterOpts](../enums/mixin.filterOpts.md)

___

### getClassByName

Re-exports [getClassByName](get_class_by_name.md#getclassbyname)

___

### getConstructor

Re-exports [getConstructor](getConstructor.md#getconstructor)

___

### getCtorOfOwnProperty

Re-exports [getCtorOfOwnProperty](getCtorOfProperty.md#getctorofownproperty)

___

### getOwnPropValue

Re-exports [getOwnPropValue](getOwnPropValue.md#getownpropvalue)

___

### getParentClass

Re-exports [getParentClass](getParentClass.md#getparentclass)

___

### getProtoChain

Re-exports [getProtoChain](getProtoChain.md#getprotochain)

___

### getPrototypeOf

Re-exports [getPrototypeOf](getPrototypeOf.md#getprototypeof)

___

### getSuper

Re-exports [getSuper](getSuper.md#getsuper)

___

### getSuperCtor

Re-exports [getSuperCtor](getSuperCtor.md#getsuperctor)

___

### hasNativeReflect

Re-exports [hasNativeReflect](isNativeReflectConstruct.md#hasnativereflect)

___

### inherits

Re-exports [inherits](inherits.md#inherits)

___

### inheritsDirectly

Re-exports [inheritsDirectly](inheritsDirectly.md#inheritsdirectly)

___

### inheritsObject

Re-exports [inheritsObject](inheritsObject.md#inheritsobject)

___

### isEmptyCtor

Re-exports [isEmptyCtor](isEmptyCtor.md#isemptyctor)

___

### isEmptyFunction

Re-exports [isEmptyFunction](isEmptyFunction.md#isemptyfunction)

___

### isEmptyFunctionCli

Re-exports [isEmptyFunctionCli](isEmptyFunction_cli.md#isemptyfunctioncli)

___

### isInheritedFrom

Re-exports [isInheritedFrom](isInheritedFrom.md#isinheritedfrom)

___

### isInheritedFromStr

Re-exports [isInheritedFromStr](isInheritedFromStr.md#isinheritedfromstr)

___

### isMixinedFrom

Re-exports [isMixinedFrom](isMixinedFrom.md#ismixinedfrom)

___

### isMixinedFromStr

Re-exports [isMixinedFromStr](isMixinedFromStr.md#ismixinedfromstr)

___

### isNativeReflectConstruct

Re-exports [isNativeReflectConstruct](isNativeReflectConstruct.md#isnativereflectconstruct)

___

### mixin

Re-exports [mixin](mixin.md#mixin)

___

### mixins

Re-exports [mixins](mixin.md#mixins)

___

### replaceCtor

Re-exports [replaceCtor](replaceCtor.md#replacector)

___

### setPrototypeOf

Re-exports [setPrototypeOf](setPrototypeOf.md#setprototypeof)
